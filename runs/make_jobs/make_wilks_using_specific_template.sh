...
OUTDIR=icarus_uboone_apponly_split_intrinsic_nodirt_nooffdiag_extended
...
do
    ...
    do
        ...
        sed -e "s|@@begin@@|${begin}|g" \
            ...
            -e "s|@@tag@@|${TAG}|g" \
            -e "s|@@det@@|${DETECTORS}|g" \
            < submit_wilks_specific_template.sh > $RUN_FOLDER/runs/scripts_${OUTDIR}/${name}.sh
        let COUNT=$COUNT+1
   done
done
echo $COUNT
